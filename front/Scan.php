			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<b>Siege</b>                   200.1.1.0/24<br>
						<b>Usine Msaken</b>            128.91.50.0/24<br>
						<b>Usine Menzel Bourguiba</b>  128.91.100.0/24<br>
						<b>Bureau Transit Sousse</b>   192.168.2.0/24<br>
						<b>Depot Ksar Said</b>         192.168.1.0/24<br>
 						</div><!-- /.page-header -->
						...
						<form class="form-horizontal"  method="POST" action="verif.php">
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Adresse IP </label>

										<div class="col-sm-9">
											<input type="text" name="addr" id="form-field-1" placeholder="@IP" class="col-xs-10 col-sm-5" />
										</div>
									</div>
						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="alert alert-info">
									...
									Liste des Scans du moi <?php echo date("n-y") ;?>.
		...
		<script type="text/javascript">
			...
			[ 
				<?php
					...
					$reponse = mysql_query("SELECT  * FROM  glpi_scan_man where  DAY(date) between '4' and '31' and MONTH(date) like '%".date("n")."%'  order by (date) desc ") or die ('erreur SQL '.$query.'<br>'.mysql_error());
		
						
						while($donnees = mysql_fetch_array($reponse)){
							
							
			//$max = mysql_query("SELECT max(date) FROM  glpi_scan_man where  adresse_ip='".$donnees['adresse_ip']."'  ") or die ('erreur SQL '.$query.'<br>'.mysql_error());
			//$donneemax = mysql_fetch_array($max);
							...
							$reponse1 = mysql_query("SELECT * FROM glpi_scan where date='".$donnees['date']."' and titre_v='".$donnees['adresse_ip']."' and  Adresse_ip = '".$donnees['adresse_ip']."'") or die ('erreur SQL '.$query.'<br>'.mysql_error());
							$Num= mysql_query("SELECT COUNT( * )FROM glpi_scan where id_plugin !=  '' and date='".$donnees['date']."'  and Adresse_ip = '".$donnees['adresse_ip']."'");
							$Num1 = mysql_fetch_array($Num);
							
								while($donnees1 = mysql_fetch_array($reponse1)){
							$nbv=$nbv+1;
							if ($donnees1['niveau_v'] == "0"){$nb0=$nb0+1;}
							if ($donnees1['niveau_v'] == "1"){$nb1=$nb1+1;}
							if ($donnees1['niveau_v'] == "2"){$nb2=$nb2+1;}
							if ($donnees1['niveau_v'] == "3"){$nb3=$nb3+1;}
							if ($donnees1['niveau_v'] == "4"){$nb4=$nb4+1;}
					       }
							...
							?>
							{
							 plugin:"<a class='red' href='delete2.php?id=<?php echo $donnees['id'];?>'>	<i class='ace-icon fa fa-trash-o bigger-130'></i></a>",
							 id:"<?php echo $donnees['id'];?>",
							 severite:"<div class=''> <div class='progress pos-rel' ><div class='progress-bar progress-bar-danger' style='width: <?php echo (($nb4*100)/$nbv); ?>%;' ></div>	<div class='progress pos-rel' ><div class='progress-bar progress-bar-warning' style='width: <?php echo (($nb3*100)/$nbv); ?>%;'></div><div class='progress pos-rel' ><div class='progress-bar progress-bar-success' style='width: <?php echo (($nb2*100)/$nbv); ?>;'></div>	<div class='progress pos-rel' >	<div class='progress-bar' style='width:<?php echo (($nb1*100)/$nbv); ?>%;'></div><div class='progress pos-rel'><div class='infobox infobox-blue  infobox-dark' style='width:<?php echo (($nb0*100)/$nbv); ?>%;'></div>	</div></div></div></div></div></div>",
							 adresse:"<a href='details.php?ip=<?php echo $donnees['adresse_ip'];?>&amp;date=<?php echo $donnees['date'];?>'><?php echo $donnees['adresse_ip'];?></a>",
							 Sites:"<?php echo $donnees['site'];?>", 
							 date:"<?php echo $donnees['date'];?>"},
								
							/*{id:"<?php echo $donnees['id'];?>",
							plugin:"<?php echo $donnees['id_plugin'];?>",
							note:"note",
							adresse:"<?php echo $donnees['adresse_ip'];?>",
							nom:"<?php echo $donnees['description_scan'];?>", 
							date:"<?php echo $donnees['date'];?>"},*/
			...
			jQuery(function($) {
				...
				*/
				...
				jQuery(grid_selector).jqGrid({
					...
					datatype: "local",
					height: 250,
					colNames:[' ', '','ID','Adresse IP', 'Sites', 'Date','severite'],
					colModel:[
						{name:'myac',index:'', width:5, fixed:false, sortable:false, resize:false,
							formatter:'actions', 
							formatoptions:{ 
								keys:false,
						...
						{name:'plugin',index:'id', width:30, sorttype:"int", editable: true},
						{name:'id',index:'plugin',width:60, editable:true, sorttype:"date",unformat: pickDate},
						{name:'adresse',index:'adresse', width:150, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
						{name:'Sites',index:'Sites', width:110, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
						{name:'date',index:'date', width:150, editable: true,edittype:"select",editoptions:{value:"FE:FedEx;IN:InTime;TN:TNT;AR:ARAMEX"}},
